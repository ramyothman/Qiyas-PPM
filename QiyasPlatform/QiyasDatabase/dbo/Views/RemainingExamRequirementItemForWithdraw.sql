...
SELECT e.ExamRequirementItemID, e.ExamCenterRequiredExamsID, e.ExamID, e.PrintsForOneModel - ISNULL(v.PrintsForOneModel, 0) AS PrintsForOneModel, e.ExamsNeededForA3 - ISNULL(v.ExamsNeededForA3, 0) 
                  ...
                  e.RequestPreparationStatusID
FROM     PPM.ExamRequirementItem AS e LEFT OUTER JOIN
                  dbo.ViewRequestWithdrawDetailSubTotal AS v ON e.ExamRequirementItemID = v.ExamRequirementItemID
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'RemainingExamRequirementItemForWithdraw';
