...
SELECT ExamRequirementItemID, SUM(PrintsForOneModel) AS PrintsForOneModel, SUM(ExamsNeededForA3) AS ExamsNeededForA3, SUM(ExamsNeededForA4) AS ExamsNeededForA4, SUM(ExamsNeededForCD) 
                  ...
                  AS ShippedExamsNeededForA4, SUM(ShippedExamsNeededForCD) AS ShippedExamsNeededForCD
FROM     PPM.RequestWithdrawDetail
GROUP BY ExamRequirementItemID
GO
EXECUTE sp_addextendedproperty @name = N'MS_DiagramPaneCount', @value = 1, @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'ViewRequestWithdrawDetailSubTotal';
